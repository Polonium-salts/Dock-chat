...
export const handleJoinRoom = async (session, roomId, contacts, setActiveChat, setShowJoinModal, showToast, updateConfig, userConfig, setUserConfig, updateChatRoomsCache, loadChatMessages) => {
  ...
  try {
    ...
    // 检查协作者状态
    const collaboratorStatus = await checkCollaboratorStatus(session.accessToken, username, session.user.login);
    
    if (collaboratorStatus === 'none') {
      // 如果不是协作者，请求添加为协作者
      try {
        // 获取聊天室所有者的访问令牌
        const ownerTokenResponse = await fetch(`/api/github/get-owner-token?username=${username}`);
        if (!ownerTokenResponse.ok) {
          throw new Error('无法获取聊天室所有者的访问令牌');
        }
        const { ownerAccessToken } = await ownerTokenResponse.json();
        
        const result = await addRepositoryCollaborator(ownerAccessToken, username, session.user.login);
        if (result.status === 'invitation_sent') {
          showToast('已发送协作者邀请，请等待聊天室所有者确认', 'info');
          return;
        }
      } catch (error) {
        console.error('Error adding collaborator:', error);
        showToast('无法加入聊天室：需要所有者授权', 'error');
        return;
      }
    } else if (collaboratorStatus === 'pending') {
      showToast('协作者邀请待处理，请等待聊天室所有者确认', 'info');
      return;
    }
