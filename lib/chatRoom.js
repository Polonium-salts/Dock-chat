  ...
  try {
    ...
    if (!roomResponse.ok) {
      ...
      const originalResponse = await fetch(
        `https://api.github.com/repos/${username}/dock-chat-data/contents/chats/${username}-${domain}/info.json`,
        {
          headers: {
            'Authorization': `Bearer ${session.accessToken}`,
            'Accept': 'application/vnd.github.v3+json'
      ...
      const roomData = await originalResponse.json();
