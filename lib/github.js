...
// 保存 AI 聊天记录
export async function saveAIChatHistory(accessToken, username, messages) {
  try {
    const config = await getConfig(accessToken, username)
    if (!config) {
      throw new Error('Config not found')
    ...
    try {
      const currentFile = await fetch(
        `https://api.github.com/repos/${username}/dock-chat-data/contents/ai_chat_history.json`,
        {
          headers: {
            'Authorization': `Bearer ${accessToken}`,
            'Accept': 'application/vnd.github.v3+json'
      ...
      if (!currentFile.message) { // 如果文件存在
        sha = currentFile.sha
      }
    } catch (error) {
      console.log('AI chat history file does not exist yet')
    }
    ...
    const response = await fetch(
      `https://api.github.com/repos/${username}/dock-chat-data/contents/ai_chat_history.json`,
      {
        ...
        body: JSON.stringify({
          message: 'Update AI chat history',
          content: content,
          ...(sha ? { sha } : {})
        })
      }
    ...
    if (!response.ok) {
      throw new Error(`Failed to save AI chat history: ${response.status}`)
    }
...
// 加载 AI 聊天记录
export async function loadAIChatHistory(accessToken, username) {
  try {
    const response = await fetch(
      `https://api.github.com/repos/${username}/dock-chat-data/contents/ai_chat_history.json`,
      {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to load AI chat history: ${response.status}`)
    }
