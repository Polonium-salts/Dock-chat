...
// 好友相关功能
export async function getFriendsList(octokit, owner) {
  try {
    const friendsFile = await getFileContent(octokit, owner, 'friends.json')
...
export async function addFriend(octokit, owner, username) {
  try {
    // 获取用户信息
    ...
    const friends = await getFriendsList(octokit, owner)
    ...
    await saveFile(octokit, owner, 'friends.json', JSON.stringify(friends, null, 2))
...
export async function removeFriend(octokit, owner, username) {
  const friends = await getFriendsList(octokit, owner)
  const newFriends = friends.filter(f => f.username !== username)
  await saveFile(octokit, owner, 'friends.json', JSON.stringify(newFriends, null, 2))
}

// 私聊相关功能
export async function getPrivateMessages(octokit, owner, friendUsername, before = null, limit = 50) {
  try {
    const chatId = [owner, friendUsername].sort().join('-')
    const messagesFile = `private-chats/${chatId}.json`
    const messages = await getFileContent(octokit, owner, messagesFile)
      .then(JSON.parse)
      .catch(() => [])
    ...
    return {
      messages: filteredMessages.slice(-limit).reverse(),
...
export async function sendPrivateMessage(octokit, owner, friendUsername, content) {
  const chatId = [owner, friendUsername].sort().join('-')
  const messagesFile = `private-chats/${chatId}.json`
  
  let messages = []
  try {
    messages = JSON.parse(await getFileContent(octokit, owner, messagesFile))
  } catch (error) {
    if (error.status !== 404) {
      throw error
    }
  ...
  await saveFile(octokit, owner, messagesFile, JSON.stringify(messages, null, 2))
