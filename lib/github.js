...
// 保存聊天记录
export async function saveChatHistory(accessToken, username, chatId, messages) {
  try {
    const response = await fetch(`https://api.github.com/repos/${username}/dock-chat-data/contents/chats/${chatId}/history.json`, {
      method: 'PUT',
      headers: {
        'Authorization': `Bearer ${accessToken}`,
        'Content-Type': 'application/json',
      ...
      body: JSON.stringify({
        message: `Update chat history for ${chatId}`,
        content: Buffer.from(JSON.stringify({
          chatId,
          messages,
          lastUpdated: new Date().toISOString()
        }, null, 2)).toString('base64'),
        sha: await getFileSha(accessToken, username, `chats/${chatId}/history.json`)
      })
    })
...
// 加载聊天历史记录
export async function loadChatHistory(accessToken, username, chatId) {
  try {
    const response = await fetch(`https://api.github.com/repos/${username}/dock-chat-data/contents/chats/${chatId}/history.json`, {
      headers: {
        'Authorization': `Bearer ${accessToken}`,
