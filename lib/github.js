...
export async function createDataRepository(accessToken, username) {
  try {
    console.log('Creating data repository...')
    ...
    console.log('Repository created, initializing structure...')
    ...
    const files = {
      ...
      'README.md': {
        content: '# Dock Chat Data\n\nThis repository stores your personal chat data for Dock Chat.'
      }
    }
    ...
    for (const [path, { content }] of Object.entries(files)) {
      ...
      if (!createFileResponse.ok) {
        console.error(`Failed to create file ${path}:`, await createFileResponse.text())
        throw new Error(`Failed to create file ${path}: ${createFileResponse.status}`)
      }
    }
  ...
  try {
    ...
    for (const file of requiredFiles) {
      ...
      const fileResponse = await fetch(
        `https://api.github.com/repos/${username}/dock-chat-data/contents/${file}`,
        {
          headers: {
            'Authorization': `Bearer ${accessToken}`,
            'Accept': 'application/vnd.github.v3+json'
    ...
    console.log('Repository exists and has required structure')
    return true
  } catch (error) {
