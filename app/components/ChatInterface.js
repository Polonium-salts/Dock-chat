...
export default function ChatInterface() {
  ...
  const [settings, setSettings] = useState({
    ...
    backgroundX: 50, // Add X position percentage
    backgroundY: 50, // Add Y position percentage
  });
  ...
  const [chatRooms, setChatRooms] = useState([
    { id: 'general', name: '通用聊天室', unread: 0, isPublic: true },
    { id: 'tech', name: '技术交流', unread: 2, isPublic: true },
    { id: 'casual', name: '休闲娱乐', unread: 0, isPublic: true }
  ]);
  ...
  const handleBackgroundImageChange = async (url) => {
    try {
      setSettings(prev => ({ ...prev, backgroundError: false }));
      if (!url) {
        setSettings(prev => ({ ...prev, backgroundImage: '', backgroundX: 50, backgroundY: 50 }));
        return;
      }
      ...
      setSettings(prev => ({ ...prev, backgroundError: true }));
    }
  };

  // Add background position change handler
  const handleBackgroundPositionChange = ({ x, y }) => {
    setSettings(prev => ({
      ...prev,
      backgroundX: x,
      backgroundY: y,
  ...
  // Add background position selector component
  const BackgroundPositionSelector = ({ value, onChange }) => {
    ...
    return (
      <div 
        ref={selectorRef}
        className="relative w-full h-32 bg-gray-100 rounded-lg overflow-hidden cursor-crosshair border border-gray-200"
      ...
      >
        <div 
          className="absolute w-4 h-4 bg-purple-500 rounded-full -translate-x-1/2 -translate-y-1/2 shadow-lg"
          style={{
            left: `${position.x}%`,
            top: `${position.y}%`,
            cursor: 'move',
  ...
  return (
    <div 
      ...
      style={{
        ...
        backgroundPosition: `${settings.backgroundX}% ${settings.backgroundY}%`,
    ...
    >
      <div 
        ...
        style={{
          ...
          opacity: settings.backgroundImage ? 1 - settings.backgroundOpacity : 1,
      ...
      <div className="relative flex flex-col md:flex-row w-full">
        <header className="md:hidden flex-none h-16 bg-white/80 border-b border-gray-200 fixed top-0 left-0 right-0 z-50">
          <div className="h-full px-4 flex items-center justify-between">
            ...
            <h1 className="text-lg font-semibold text-gray-900">
              {activeTab === 'chat' ? chatRooms.find(room => room.id === currentRoom)?.name : 
        ...
        <nav className={`flex-none md:w-16 h-16 md:h-full bg-white/80 border-t md:border-t-0 md:border-r border-gray-200 fixed bottom-0 md:relative w-full z-50 transition-transform duration-300 ${
          ...
          <div className="h-full flex md:flex-col items-center justify-around md:justify-start md:py-4">
            ...
            <div className="flex md:flex-col gap-2 items-center justify-around w-full md:w-auto">
              ...
              >
                ...
                <span className="text-xs mt-1 hidden"></span>
              </button>
              <button
        ...
        <aside className={`flex-none w-full md:w-72 border-r border-gray-200 bg-white/80 fixed md:relative z-40 transition-transform duration-300 transform ${
        ...
        <main className="flex-1 flex flex-col h-[calc(100vh-8rem)] md:h-screen overflow-hidden mt-16 md:mt-0 mb-16 md:mb-0">
          <header className="flex-none h-16 bg-white/80 border-b border-gray-200">
                  ...
                  {activeSettingSection === 'appearance' && (
                    <div className="p-6 border-b border-gray-200">
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                          <label className="text-sm font-medium text-gray-700">{translate('appearance.backgroundImage')}</label>
                          <div className="flex items-center space-x-2">
                            <input
                              type="url"
                              value={settings.backgroundImage}
                              onChange={(e) => handleBackgroundImageChange(e.target.value)}
                              placeholder={translate('appearance.backgroundImageUrl')}
                              className={`flex-1 px-3 py-2 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 bg-white text-gray-900 ${
                                settings.backgroundError ? 'border-red-500' : 'border-gray-200'
                              }`}
                            />
                            {settings.backgroundImage && (
                              ...
                              >
                                {translate('appearance.removeBackground')}
                              </button>
                            )}
                          </div>
                          {settings.backgroundError && (
                            <p className="text-sm text-red-600">
                              {translate('appearance.backgroundImageError')}
                            </p>
                          )}
                        </div>
                        ...
                        {settings.backgroundImage && (
                          <>
                            <div className="space-y-2">
                              <label className="text-sm font-medium text-gray-700">{translate('appearance.backgroundOpacity')}</label>
                              <input
                                type="range"
                                min="0"
                                max="1"
                                ...
                                onChange={(e) => handleSaveSettings({ ...settings, backgroundOpacity: parseFloat(e.target.value) })}
                                className="w-full"
                              />
                            </div>

                            <div className="space-y-2">
                              <label className="text-sm font-medium text-gray-700">{translate('appearance.backgroundBlur')}</label>
                              <input
                                type="range"
                                min="0"
                                max="20"
                                ...
                                onChange={(e) => handleSaveSettings({ ...settings, backgroundBlur: parseInt(e.target.value) })}
                                className="w-full"
                              />
                            </div>

                            <div className="space-y-2">
                              <label className="text-sm font-medium text-gray-700">{translate('appearance.backgroundPosition')}</label>
                              <BackgroundPositionSelector
                                value={{ x: settings.backgroundX, y: settings.backgroundY }}
                                onChange={handleBackgroundPositionChange}
                              />
                              <div className="flex justify-between text-xs text-gray-500 mt-1">
                                <span>X: {Math.round(settings.backgroundX)}%</span>
                                <span>Y: {Math.round(settings.backgroundY)}%</span>
                              </div>
                            </div>

                            <div className="space-y-2">
                              <label className="text-sm font-medium text-gray-700">{translate('appearance.backgroundSize')}</label>
                              <select
                                value={settings.backgroundSize}
                                onChange={(e) => handleSaveSettings({ ...settings, backgroundSize: e.target.value })}
                                className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 bg-white text-gray-900"
                              >
                                <option value="cover">{translate('appearance.sizes.cover')}</option>
                                <option value="contain">{translate('appearance.sizes.contain')}</option>
                                <option value="auto">{translate('appearance.sizes.auto')}</option>
                              </select>
                            </div>
                          </>
                        )}
